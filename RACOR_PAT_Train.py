...
from Fully_Dense_UNet_convLSTM_bridge import getModel
from tensorflow.keras.callbacks import ModelCheckpoint,ReduceLROnPlateau
from utils import data_gen,model_loss,SSIM,PSNR,KLDivergence,plot_history,SavingMetric
...
train_filename = '/home/praveenb001/Multi_ust/Dataset_train_test_valid_4/train/train_set.npz'

########################################################################################################################
''' Choose the validation file name'''
########################################################################################################################

valid_filename =  '/home/praveenb001/Multi_ust/Dataset_train_test_valid_4/val/valid_set.npz'

########################################################################################################################
'''Provide the filename where the models have to be stored'''
########################################################################################################################

model_dir = 'Model_RACOR'
...
bestmodel_callbacks = ModelCheckpoint(filepath = os.path.join(bestmodel_dir, 'saved_model.epoch_{epoch:02d}-SSIM_{val_SSIM:.5f}-PSNR_{val_PSNR:.5f}-metric_{val_SavingMetric:.5f}.h5'), monitor='val_SavingMetric', verbose=0, save_best_only=False, save_weights_only=False, mode='min', save_freq='epoch')
